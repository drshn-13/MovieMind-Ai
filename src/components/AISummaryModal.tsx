...
export const AISummaryModal: React.FC<AISummaryModalProps> = ({ movie, isOpen, onClose }) => {
  ...
  const handleGenerate = async (
    ...
    try {
      ...
      setError(err.message || 'Failed to generate AI summary. Please check connection and try again.');
    } finally {
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(`${movie.title} (${movie.releaseYear}) - AI Summary [${length.toUpperCase()}]\n\n${summary.content}`);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  const handleListenAudio = async (forceRegenerateAudio: boolean = false) => {
    if (!summary || !movie) return;
    ...
    try {
      ...
      const estimatedWords = contentText ? contentText.split(/\s+/).filter(Boolean).length : 0;
      playTrack({
        summaryId: summary.id,
        movieId: movie.id,
        movieTitle: movie.title,
        ...
        voiceName: voiceChoice,
        durationSeconds: Math.max(15, Math.round(estimatedWords / 2.2)),
      });
    } finally {
  ...
  const isThisSummaryPlaying = Boolean(
    ...
    (currentTrack.summaryId === summary?.id || currentTrack.summaryText === summary?.content)
  );
      ...
      <div className="relative w-full max-w-2xl bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-white tracking-tight">AI Movie Summary & Narration</h2>
                <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-blue-950 text-blue-400 border border-blue-800/50">
                  Gemini 3.7 Flash
                </span>
              </div>
              <p className="text-xs text-zinc-400 truncate max-w-sm">{movie.title} ({movie.releaseYear})</p>
            </div>
          </div>
            ...
            {[
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setLength(len);
                  handleGenerate(len, isSpoilerFree, false);
                }}
          ...
          <div className="flex items-center space-x-1.5">
            <button
              onClick={() => {
                ...
                handleGenerate(length, next, false);
              }}
            ...
            >
              {isSpoilerFree ? <ShieldCheck className="w-3.5 h-3.5" /> : <ShieldAlert className="w-3.5 h-3.5" />}
              <span>{isSpoilerFree ? 'Spoiler-Free' : 'Full Story (With Spoilers)'}</span>
            </button>
          </div>
        </div>
          ...
          {isLoading ? (
            <div className="py-12 flex flex-col items-center justify-center space-y-4 text-center">
              ...
              <div className="space-y-1">
                <p className="text-sm font-semibold text-zinc-200">
                  {length === 'detailed' 
                    ? 'Crafting Detailed Chronological Storyline with Gemini AI...' 
                    : length === 'quick' 
                    ? 'Synthesizing Main Plot Events with Gemini AI...' 
                    : 'Analyzing Movie with Gemini AI...'}
                </p>
                <p className="text-xs text-zinc-500">Extracting narrative structure, character arcs, and cinematic tone</p>
              </div>
            ...
            <div className="p-5 rounded-xl bg-red-950/30 border border-red-900/50 text-center space-y-3">
              ...
              <div className="flex items-center justify-center space-x-2 pt-1">
                <button
                  onClick={() => handleGenerate(length, isSpoilerFree, true)}
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {summary.keyThemes && summary.keyThemes.map((theme, i) => (
                    <span key={i} className="px-2 py-0.5 rounded-full bg-zinc-900/80 border border-zinc-800/80 text-[11px] text-zinc-300">
                      #{theme}
                    </span>
                  ))}
                </div>
                ...
                <span className="text-[11px] font-mono text-zinc-400 bg-zinc-900 px-2 py-0.5 rounded border border-zinc-800">
                  {length === 'quick' ? 'Short Summary' : length === 'detailed' ? 'Detailed Full Story' : 'Standard Summary'}
                </span>
              </div>

              {/* Audio Status Banner if active / playing */}
              {isThisSummaryPlaying && (
                  ...
                  <div className="flex items-center space-x-2.5">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-blue-200">
                        {isPlaying ? 'Now Playing AI Audio Narration' : 'Audio Narration Paused'}
                      </p>
                      <p className="text-[11px] text-blue-300/70">Voice: {currentTrack?.voiceName || voiceChoice}</p>
                    </div>
                  </div>
              ...
              {/* If an older length audio is currently playing in background */}
              {isOldAudioPlaying && (
                <div className="p-3 rounded-xl bg-amber-950/40 border border-amber-800/50 flex items-center justify-between gap-3 animate-in fade-in">
                  <div className="flex items-center space-x-2 text-amber-300 text-xs">
                    ...
                    <span>Previous summary audio is currently playing.</span>
                  </div>
                  <button
                  ...
                  >
                    Play This {length === 'detailed' ? 'Detailed' : length === 'quick' ? 'Short' : 'Standard'} Audio
                  </button>
                </div>
              )}
              ...
              {summary.recommendedFor && (
                <div className="p-3.5 rounded-xl bg-zinc-900/80 border border-zinc-800 flex items-start space-x-2.5">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-zinc-200">Recommended For:</p>
                    <p className="text-xs text-zinc-400 mt-0.5">{summary.recommendedFor}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="px-6 py-4 border-t border-zinc-800 bg-zinc-900/80 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              <option value="Kore">Narrator (Kore - Warm)</option>
              <option value="Fenrir">Narrator (Fenrir - Deep)</option>
              <option value="Puck">Narrator (Puck - Upbeat)</option>
              <option value="Zephyr">Narrator (Zephyr - Smooth)</option>
            </select>

            {/* Listen / Regenerate Audio Button */}
            {isGeneratingAudio ? (
              <div className="flex items-center space-x-2">
                <button
                  disabled
              ...
              >
                ...
                <span>Re-listen ({voiceChoice})</span>
              </button>
            ) : (
              <button
              ...
              >
                <Headphones className="w-4 h-4" />
                <span>Listen to {length === 'detailed' ? 'Detailed' : length === 'quick' ? 'Short' : 'Standard'} Audio</span>
              </button>
            )}
